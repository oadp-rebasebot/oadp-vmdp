...
func (c *serverFlags) setup(svc appServices, cmd *kingpin.CmdClause) {
	...
	cmd.Flag("server-username", "HTTP server username (basic auth)").Envar(svc.EnvName("OADP_SERVER_USERNAME")).Default("oadp-vmdp").StringVar(&c.serverUsername)
	cmd.Flag("server-password", "HTTP server password (basic auth)").Envar(svc.EnvName("OADP_SERVER_PASSWORD")).StringVar(&c.serverPassword)
}
...
func (c *serverClientFlags) setup(svc appServices, cmd *kingpin.CmdClause) {
	...
	cmd.Flag("address", "Address of the server to connect to").Envar(svc.EnvName("OADP_SERVER_ADDRESS")).Default("http://127.0.0.1:51515").StringVar(&c.serverAddress)
	cmd.Flag("server-control-username", "Server control username").Envar(svc.EnvName("OADP_SERVER_USERNAME")).StringVar(&c.serverUsername)
	cmd.Flag("server-control-password", "Server control password").PlaceHolder("PASSWORD").Envar(svc.EnvName("OADP_SERVER_PASSWORD")).StringVar(&c.serverPassword)
	...
	cmd.Flag("server-cert-fingerprint", "Server certificate fingerprint").PlaceHolder("SHA256-FINGERPRINT").Envar(svc.EnvName("OADP_SERVER_CERT_FINGERPRINT")).StringVar(&c.serverCertFingerprint)
}
